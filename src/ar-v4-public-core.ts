...
export type PublicReference={id:string;nodeId:string;imageUrl:string;bearing:number|null;packUrl?:string;packError?:string};
...
export function publicReferences(nodes:any[]):PublicReference[]{
 ...
 for(const n of nodes.filter((n,i,a)=>n&&a.findIndex(x=>x?.id===n.id)===i)){
  if(Array.isArray(n.recognitionRefs)){
   for(const r of n.recognitionRefs)result.push({id:r.id,nodeId:n.id,imageUrl:'',bearing:optional(r.bearing),packUrl:r.packUrl,packError:r.error});
   continue;
  }
