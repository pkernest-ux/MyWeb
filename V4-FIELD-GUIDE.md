...
- 工作台與民眾端相機辨識都按「同一節點」連續 3 次確認；0°、45° 等不同照片交替匹配可以接續。未匹配、不同節點或相隔超過 5 秒會重算；幾何驗證與歧義檢查未放寬。工作台匯入單張測試照仍是單次測試，不等於相機連續確認。
...
3. 持續比對目前路段起終點、沿途及同樓層直接相鄰節點的 V4 觀測照片（包括環景拆圖），並保留舊參考照片相容性。連續三次匹配同一節點後才提出候選；搜尋上限與照片載入狀態見「辨識診斷與搜尋範圍」。
