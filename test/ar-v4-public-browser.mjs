...
// Real worker recognition of nodes omitted by the old endpoint-only scope.
 ...
 await failed.getByRole('button',{name:'導引說明與校正',exact:true}).click();await failed.getByText('辨識／方向需要協助',{exact:true}).click();assert.equal(await failed.getByRole('button',{name:'人工確認已到此地標'}).isEnabled(),true);
